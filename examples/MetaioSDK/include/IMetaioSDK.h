...
public:
	 ...
	 */
	static stlcompat::String getVersion();
	...
	*/
	 ...
	 *
	 * \sa setHandEyeCalibrationFromFile
	 ...
	 *
	 * If the file does not exist or cannot be parsed, nothing is changed and false is returned.
	 *
	 * The calibration and XML file can be created and exported with Toolbox on supported devices (e.g. Epson Moverio).
	 *
	 * \param calibrationFilePath	XML file path. If empty (default), the method tries to read the default file path
	 *								as created in Toolbox. This way, you can allow your users to use Toolbox in order
	 *								to create an exact calibration.
	 * \sa setHandEyeCalibrationFromFile
	 * \return True if a calibration was set, false if nothing was changed because file not existing or invalid
	 */
	virtual bool setHandEyeCalibrationFromFile(const metaio::Path& calibrationFilePath = metaio::Path()) = 0;
    ...
    /**
     * Set which cameras to render in stereo mode (default is left and right eye)
     * \param cameraTypes Bitwise combination of ECT_RENDERING_STEREO_LEFT, ECT_RENDERING_STEREO_RIGHT, or none
     */
    virtual void setStereoRenderingMask(unsigned int cameraTypes) = 0;
	 ...
	 * 
	 * When envMapFormat is set to EEMF_CUBESIDES, six image files have to be supplied. This is the
	 * default format.
	 *
	 ...
	 *					of the cube. In case of envMapFormat = EEMF_LATLONG, this must be set to

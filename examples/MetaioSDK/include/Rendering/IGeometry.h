...
public:
	 ...
	 * myGeometry->setBillboardModeEnabled(true);
	 * 
	 * // disable billboard mode
	 * myGeometry->setBillboardModeEnabled(false); 
	 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	 *    
	 ...
	 *
	 * The z-buffer (depth) check can be optionally disabled so that geometries are rendered independent of their
	 * real distance from the camera. This is useful when more than one geometries are positioned at same depth (z), or
	 ...
	 */
	virtual bool setCustomAnimation(const stlcompat::String& name, 
									const CustomAnimation& animation) = 0;
	 ...
	 */
	virtual CustomAnimation getCustomAnimation(const stlcompat::String& name) const = 0;
	 ...
	 */
	virtual bool removeCustomAnimation(const stlcompat::String& name) = 0;
	 ...
	 */
	virtual float getCurrentFrame(EANIMATION_TYPE animType = EAT_MODEL_ONLY) const = 0;
	...
	/**
	 * Sets a movie as a texture for the geometry.
	 *
	 ...
	 * \param transparent If true, the movie file will be rendered as transparent texture.
	 * \sa removeMovieTexture
	 * \return true if successful, false otherwise (e.g. file not found or not loadable).
	 */
	...
	/**
	 * Remove the movie texture from the geometry.
	 * \sa setMovieTexture
	 */
	 ...
	 * If the movie texture was stopped, it will start from beginning. If it was paused,

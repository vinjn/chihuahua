	...
	public void draw(GL10 gl, ESCREEN_ROTATION screenRotation) {
		...
		if (mMustUpdateTexture) {
			if (!mTextureInitialized) {
				...
				mTexturePtr = UGraphics.Scene_addEmptyTexture(mCameraW,
						mCameraH);
				Log.e(TAG, "Init camera texture: " + mCameraW + ", " + mCameraH);

				if (mIsNodeMode) {
					long node = UGraphics.Scene_addFullScreenTextureNode(
							mTexturePtr, screenRotation.ordinal());
			...
			UGraphics.Texture_update(mTexturePtr, jImageData);
			mMustUpdateTexture = false;
		}

		if (!mIsNodeMode) {
			UGraphics.Texture_renderFullScreen(mTexturePtr,

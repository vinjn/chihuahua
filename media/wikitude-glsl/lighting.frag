...
vec3 computeLighting(vec3 normalVector, vec3 lightDirection, float attenuation, vec3 cameraDirection)
{
    ...
    float specularIntensity = attenuation * max(0.0, pow(abs(dot(normalVector, halfVector)), u_specularExponent));
    specularIntensity = max(0.0, specularIntensity);
    _specularColor = u_lightColor * u_specularColor.rgb * specularIntensity;

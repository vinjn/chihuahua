...
void CAnimatedMeshSceneNode::buildFrameNr(u32 timeMs)
{
	...
	// printf("timeMs: %d, CurrentFrameNr: %.1f", timeMs, CurrentFrameNr);
}
...
bool CAnimatedMeshSceneNode::setAnimation(u32 anim)
{
	if (!Mesh)
		return false;

	s32 begin, end, speed;
	if (!Mesh->getFrameLoop(anim, begin, end, speed))
		return false;
...
bool CAnimatedMeshSceneNode::setAnimationByName(const c8* animationName)
{
	...
	s32 begin, end, speed;
	if (!Mesh->getFrameLoop(animationName, begin, end, speed))
		return false;

...
#define GL_UNSIGNED_INT 0x1405
		...
		switch (pType)
		{
			...
			case scene::EPT_TRIANGLES:
				...
				break;
			default:
				os::Printer::log("Unsupported primitive type", ELL_ERROR);
				break;
		}
	...
	//! The same, but with a four element array of colors, one for each vertex
	void COGLES2Driver::draw2DImageRotated(const video::ITexture* texture,
			const core::rect<s32>& destRect,
			...
			const video::SColor* const colors, bool useAlphaChannelOfTexture)
	{
		...
		{
			core::vector3df(left, top, 0),
			core::vector3df(right, top, 0),
			core::vector3df(right, down, 0),
			core::vector3df(left, down, 0)
		};
...
/*#if defined(GL_OES_depth24)
	...
	{
        //printf("%d = %d\n", ActiveTextureID, texture ? texture->getOpenGLTextureName() : 0);

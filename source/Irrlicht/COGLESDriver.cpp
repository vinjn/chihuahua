...
#endif
{
	...
	if (ContextManager)
	{
		// We should support surface and gl context mantained by other environment, such as
		// GLSurfaceView in Android SDK
		ContextManager->grab();
...
bool COGLES1Driver::genericDriverInit(const core::dimension2d<u32>& screenSize, bool stencilBuffer)
{
	...
	// printf("initExtensions");testGLError();

	if (!BridgeCalls)
		BridgeCalls = new COGLES1CallBridge(this);

	// printf("bridge");testGLError();
	...
	// printf("store");testGLError();

	// Reset The Current Viewport
	glViewport(0, 0, screenSize.Width, screenSize.Height);

	// printf("viewport");testGLError();
	...
	// printf("setTransform");testGLError();
...
//	glLightModeli(GL_LIGHT_MODEL_LOCAL_VIEWER, 1);
	...
	// printf("front");testGLError();


	// create material renderers
	createMaterialRenderers();

	// printf("MaterialRenderers");testGLError();

	// set the renderstates
	setRenderStates3DMode();

	// printf("setRenderStates3DMode");testGLError();

	glAlphaFunc(GL_GREATER, 0.f);

	// printf("alpha");testGLError();

	// set fog mode
	setFog(FogColor, FogType, FogStart, FogEnd, FogDensity, PixelFog, RangeFog);
	// printf("fog");testGLError();
...
// vinjn: add for Metaio camera stream rendering
//! The same, but with a four element array of colors, one for each vertex
void COGLES1Driver::draw2DImageRotated(const video::ITexture* texture, const core::rect<s32>& destRect,
		const core::rect<s32>& sourceRect, int rotateType, const core::rect<s32>* clipRect,
		const video::SColor* const colors, bool useAlphaChannelOfTexture)
{
	if (!texture)
		return;
    ...
    {
		core::vector3df((f32)destRect.UpperLeftCorner.X, (f32)destRect.UpperLeftCorner.Y, 0),
		core::vector3df((f32)destRect.LowerRightCorner.X, (f32)destRect.UpperLeftCorner.Y, 0),
		core::vector3df((f32)destRect.LowerRightCorner.X, (f32)destRect.LowerRightCorner.Y, 0),
		core::vector3df((f32)destRect.UpperLeftCorner.X, (f32)destRect.LowerRightCorner.Y, 0)
	};

	core::vector2df texcoords[] =
	{
		core::vector2df(tcoords.UpperLeftCorner.X, tcoords.UpperLeftCorner.Y),
		core::vector2df(tcoords.LowerRightCorner.X, tcoords.UpperLeftCorner.Y),
		core::vector2df(tcoords.LowerRightCorner.X, tcoords.LowerRightCorner.Y),
		core::vector2df(tcoords.UpperLeftCorner.X, tcoords.LowerRightCorner.Y)
	};
	...
	u8 useColorMappings[] = {0, 3, 2, 1};
	u16 indices[] = {0,1,2,3};
	S3DVertex vertices[4];
	for (int i = 0; i < 4; ++i)
	{
		vertices[i].Pos = positions[i];
		vertices[i].TCoords = texcoords[(i+rotateType) % 4];
		vertices[i].Normal.set(0,0,1);
		vertices[i].Color = useColor[useColorMappings[i]];
	}
	drawVertexPrimitiveList2d3d(vertices, 4, indices, 2, video::EVT_STANDARD, scene::EPT_TRIANGLE_FAN, EIT_16BIT, false);
...
bool COGLES1Driver::setActiveTexture(u32 stage, const video::ITexture* texture)
{
	...
	else
	{
		...
		GLuint tex = static_cast<const COGLES1Texture*>(texture)->getOGLES1TextureName();
...
void COGLES1Driver::setRenderStates3DMode()
{
	if (CurrentRenderMode != ERM_3D)
	{
		...
		// printf("setBlendFunc");testGLError();

		// switch back the matrices
		glMatrixMode(GL_MODELVIEW);
		glLoadMatrixf((Matrices[ETS_VIEW] * Matrices[ETS_WORLD]).pointer());

		// printf("glLoadMatrixf");testGLError();
		...
		// printf("glLoadMatrixf2");testGLError();
	...
	if ( ResetRenderStates || LastMaterial != Material)
	{
				...
				static_cast<u32>(LastMaterial.MaterialType) < MaterialRenderers.size())
		{
			...
			// printf("OnUnsetMaterial");testGLError();
		}
		...
		if (static_cast<u32>(Material.MaterialType) < MaterialRenderers.size())
		{
			...
			// printf("OnSetMaterial");testGLError();
		}
	...
	bool resetAllRenderStates)
{
		...
		lastmaterial.ColorMaterial != material.ColorMaterial)
	{
		...
		if (material.ColorMaterial == ECM_DIFFUSE_AND_AMBIENT)
		{
			...
			// // printf("glEnable"); testGLError();
		}
		else
		{
			glDisable(GL_COLOR_MATERIAL);
			// // printf("glDisable"); testGLError();
		}
	}
		...
		lastmaterial.ColorMaterial != material.ColorMaterial)
	{
		...
		if (material.ColorMaterial != video::ECM_EMISSIVE)
		{
			...
			// // printf("EmissiveColor"); testGLError();
		}
	}
		...
		lastmaterial.Shininess != material.Shininess)
	{
			...
			(material.ColorMaterial != video::ECM_SPECULAR))
		{
			...
			if (FeatureAvailable[IRR_EXT_separate_specular_color])
			{
				...
				// // printf("separate specular"); testGLError();
			}
#endif
	}
	...
	for (u32 i=0; i<MaxTextureUnits; ++i)
	{
		...
		if (FeatureAvailable[IRR_EXT_texture_lod_bias])
		{
			if (material.TextureLayer[i].LODBias)
			{
				...
				// // printf("GL_TEXTURE_FILTER_CONTROL_EXT"); testGLError();
			}
			else
			{
				glTexEnvf(GL_TEXTURE_FILTER_CONTROL_EXT, GL_TEXTURE_LOD_BIAS_EXT, 0.f);
				// // printf("GL_TEXTURE_FILTER_CONTROL_EXT2"); testGLError();
			}
		}
#endif
		...
		// // printf("glTexParameteri"); testGLError();
		...
		if (material.getTexture(i) && material.getTexture(i)->hasMipMaps())
					...
					(CurrentTexture[i]->getSize() != CurrentTexture[i]->getOriginalSize()))
			{
				glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
					...
					GL_NEAREST);
				// // printf("GL_TEXTURE_MIN_FILTER GL_NEAREST"); testGLError();
			}
			else
			{
				glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
				...
				// // printf("GL_TEXTURE_MIN_FILTER GL_NEAREST_MIPMAP_NEAREST"); testGLError();
			}
		else
		{
			glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
		...
		if (FeatureAvailable[IRR_EXT_texture_filter_anisotropic])
		{
			...
			// // printf("GL_TEXTURE_MAX_ANISOTROPY_EXT"); testGLError();
		}
#endif
	}
...
//		glPolygonMode(GL_FRONT_AND_BACK, material.Wireframe ? GL_LINE : material.PointCloud? GL_POINT : GL_FILL);
	...
	if (resetAllRenderStates || (lastmaterial.GouraudShading != material.GouraudShading))
	{
		...
		// // printf("glShadeModel"); testGLError();
	}
	...
	if (resetAllRenderStates || (lastmaterial.Lighting != material.Lighting))
	{
		...
		// // printf("GL_LIGHTING"); testGLError();
	}
	...
	// // printf("big switch-case"); testGLError();
...
//	if (resetAllRenderStates || lastmaterial.ZWriteEnable != material.ZWriteEnable)
	{
		...
		// // printf("glDepthMask"); testGLError();
	}
	...
	if (resetAllRenderStates || lastmaterial.NormalizeNormals != material.NormalizeNormals)
	{
		...
		// // printf("GL_NORMALIZE"); testGLError();
	}
	...
	if (resetAllRenderStates || lastmaterial.ColorMask != material.ColorMask)
	{
		...
		// // printf("glColorMask"); testGLError();
	}
	...
	// // printf("BridgeCalls"); testGLError();
...
//		if (FeatureAvailable[IRR_ARB_multisample])
	...
	// // printf("AA"); testGLError();

	setWrapMode(material);
	// // printf("setWrapMode"); testGLError();

...
// These override the ue::KEY_FOO equivalents, which stops key handling from working.
	...
	if ((CreationParams.DriverType != video::EDT_NULL) && XDisplay)
	{
		...
		while (XPending(XDisplay) > 0 && !Close)
		{
			...
			case MotionNotify:
				...
				// mouse button states
				irrevent.MouseInput.ButtonStates = (event.xbutton.state & Button1Mask) ? ue::EMBSM_LEFT : 0;
				irrevent.MouseInput.ButtonStates |= (event.xbutton.state & Button3Mask) ? ue::EMBSM_RIGHT : 0;
				irrevent.MouseInput.ButtonStates |= (event.xbutton.state & Button2Mask) ? ue::EMBSM_MIDDLE : 0;
			...
			case ButtonRelease:
				...
				irrevent.MouseInput.Event = ue::EMIE_COUNT;

				switch(event.xbutton.button)
				{
				case  Button1:
					irrevent.MouseInput.Event =
						(event.type == ButtonPress) ? ue::EMIE_LMOUSE_PRESSED_DOWN : ue::EMIE_LMOUSE_LEFT_UP;
					irrevent.MouseInput.ButtonStates ^= ue::EMBSM_LEFT;
					break;

				case  Button3:
					irrevent.MouseInput.Event =
						(event.type == ButtonPress) ? ue::EMIE_RMOUSE_PRESSED_DOWN : ue::EMIE_RMOUSE_LEFT_UP;
					irrevent.MouseInput.ButtonStates ^= ue::EMBSM_RIGHT;
					break;

				case  Button2:
					irrevent.MouseInput.Event =
						(event.type == ButtonPress) ? ue::EMIE_MMOUSE_PRESSED_DOWN : ue::EMIE_MMOUSE_LEFT_UP;
					irrevent.MouseInput.ButtonStates ^= ue::EMBSM_MIDDLE;
					break;
				...
				if (irrevent.MouseInput.Event != ue::EMIE_COUNT)
				{
					postEventFromUser(irrevent);
...
void CIrrDeviceLinux::setWindowSize(const ue::core::dimension2d<u32>& size)
{
#ifdef _IRR_COMPILE_WITH_X11_
	if (CreationParams.DriverType == video::EDT_NULL || CreationParams.Fullscreen )
		return;
...
bool CIrrDeviceLinux::activateJoysticks(core::array<SJoystickInfo> & joystickInfo)
{
	...
	for (joystick = 0; joystick < 32; ++joystick)
	{
		...
		info.persistentData.EventType = ue::EET_JOYSTICK_INPUT_EVENT;
...
Cursor CIrrDeviceLinux::TextureToMonochromeCursor(ue::video::ITexture * tex, const core::rect<s32>& sourceRect, const core::position2d<s32> &hotspot)
{
...
#ifdef _IRR_LINUX_XCURSOR_
Cursor CIrrDeviceLinux::TextureToARGBCursor(ue::video::ITexture * tex, const core::rect<s32>& sourceRect, const core::position2d<s32> &hotspot)
{
...
Cursor CIrrDeviceLinux::TextureToCursor(ue::video::ITexture * tex, const core::rect<s32>& sourceRect, const core::position2d<s32> &hotspot)
{
#ifdef _IRR_LINUX_XCURSOR_
...
ue::core::dimension2di CIrrDeviceLinux::CCursorControl::getSupportedIconSize() const
{
